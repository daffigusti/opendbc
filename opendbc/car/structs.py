    ...
    for name, typ in cls_annotations.items():
        ...
        if current_value is AUTO_OBJ:
            ...
            if isinstance(origin_typ, str):
                raise TypeError(
                    f"Forward references are not supported for auto_field: '{origin_typ}'. Use a default_factory with lambda instead."
                )
            elif origin_typ in (
                int,
                float,
...
class CarParamsSP:
    ...
    neuralNetworkLateralControl: "CarParamsSP.NeuralNetworkLateralControl" = field(
        default_factory=lambda: CarParamsSP.NeuralNetworkLateralControl()
    )

    @auto_dataclass
    class NeuralNetworkLateralControl:
        model: "CarParamsSP.NeuralNetworkLateralControl.Model" = field(
            default_factory=lambda: CarParamsSP.NeuralNetworkLateralControl.Model()
        )
...
@auto_dataclass
class CarControlSP:
    mads: "ModularAssistiveDrivingSystem" = field(
        default_factory=lambda: ModularAssistiveDrivingSystem()
    )
